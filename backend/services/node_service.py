...
def get_nodes(current_user: User) -> List[Dict[str, Any]]:
    ...
    for record in raw_nodes:
        ...
        if node.get("location"):
            try:
                if hasattr(node.get("location"), 'latitude'):
                    loc = {"lat": node.get("location").latitude, "long": node.get("location").longitude}
            except: pass
        ...
        for m in record["metrics"]:
            if m.get("name"): # Filter out nulls from OPTIONAL MATCH
                ...
                if m.get("last_updated"):
                    try: m_data["last_updated"] = m["last_updated"].iso_format()
                    except: pass
        ...
        node_data = {
            ...
            "metadata": {
                **clean_metadata,
                "locationName": node.get("location_name")
            },
            "metrics": metrics
        }
        # Parse SNMP stored as string if necessary
        if isinstance(node_data["snmp"], str):
                try: node_data["snmp"] = json.loads(node_data["snmp"])
                except: node_data["snmp"] = None # Avoid Pydantic Validation Error
...
async def bulk_upload_nodes(file_contents: bytes, filename: str) -> JSONResponse:
    ...
    for index, row in df.iterrows():
        ...
        if pd.isna(row.get('Label')): continue
            
        label = str(row['Label']).strip()
        raw_id = str(row.get('ID', '')).strip()

        if label == "Sample Router" or raw_id == "(Leave Empty for Auto-ID)": continue
        ...
        if ntype != 'INFRASTRUCTURE' and ntype not in valid_layers:
                validation_errors.append(f"Row {row_idx} (ID: {nid}): NetworkLayer '{ntype}' not found.")
                continue
        
        # Normalize Status
        status_upper = status.upper()
        if status_upper in ['HEALTHY', 'OK', 'ACTIVE']: status = 'ACTIVE'
        elif status_upper in ['WARNING', 'CRITICAL', 'EXCEPTION']: status = 'EXCEPTION'
        elif status_upper == 'MAINTENANCE': status = 'MAINTENANCE'
        else: status = 'ACTIVE'
        ...
        topology_repo.bulk_insert_node(
            nid, label, ntype, status, ip, 
            brand, model, serial, firmware,
            lat, long, 60, snmp_str,
            metadata, owner
        )
        processed_count += 1
...
def get_node_template() -> StreamingResponse:
    ...
    template_data = {
        "ID": ["(Leave Empty for Auto-ID)"], "Label": ["Sample Router"], "NetworkLayer": ["INFRASTRUCTURE"],
        "OperationalStatus": ["ACTIVE"], "Brand": ["Cisco"], "Model": ["ASR-1000"],
        "SerialNumber": ["SN12345678"], "Firmware": ["17.3.1"], "IP": ["192.168.1.100"],
        "SNMP_Version": ["v2c"], "SNMP_Read": ["public"], "SNMP_Write": ["private"],
        "Owner": ["NetOps"], "Location": ["Data Center A"],
        "Latitude": [19.4326], "Longitude": [-99.1332], "Criticality": ["High"]
    }
    
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        pd.DataFrame(template_data).to_excel(writer, index=False, sheet_name='Data Entry Template')
        pd.DataFrame(owners_list, columns=["Available Owners"]).to_excel(writer, index=False, sheet_name='Ref - Owners')
        pd.DataFrame(layers_list, columns=["Available Network Layers"]).to_excel(writer, index=False, sheet_name='Ref - Network Layers')
