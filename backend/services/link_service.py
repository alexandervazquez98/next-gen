...
def get_links(current_user: User | None = None) -> list[dict[str, Any]]:
    """
...
def validate_tunnel_endpoint_hub(
    ...
    if medium not in ALLOWED_TUNNEL_MEDIUMS:
        raise HTTPException(
            status_code=400,
            detail=(f"medium must be one of {sorted(ALLOWED_TUNNEL_MEDIUMS)}; got {medium!r}"),
        )
...
def get_full_graph(
    ...
    for node_props in raw_nodes:
        ...
        elif "OwnerGroup" in labels:
            primary_type = "Owner"
        elif "MetricDef" in labels:
            primary_type = "Metric"
        elif "HardwareModel" in labels:
            primary_type = "Hardware"
        elif "User" in labels:
            primary_type = "User"
        ...
        fallback_id = (
            ...
            or " ".join(
                str(part) for part in [node_props.get("brand"), node_props.get("model")] if part
            )
        )
...
def simulate_bulk_links(
    current_user: User, source_filter: dict, target_filter: dict
) -> dict[str, Any]:
    """
    ...
    if not is_admin:
        if (
            source_filter.get("location")
            and source_filter["location"] not in current_user.allowed_locations
        ):
            raise ValueError(f"Unauthorized location in source filter: {source_filter['location']}")
        if (
            target_filter.get("location")
            and target_filter["location"] not in current_user.allowed_locations
        ):
    ...
    src_ids, tgt_ids = _get_filtered_node_ids(
        source_filter, target_filter, is_admin, current_user.allowed_locations
    )
    ...
    return {
        ...
        "message": (
            "Ready to execute"
            if count <= 500
            else "Too many potential links (> 500). Please refine filters."
        ),
...
def _get_filtered_node_ids(
    source_filter: dict, target_filter: dict, is_admin: bool, allowed_locations: list | None
...
def execute_bulk_links(
    current_user: User, source_filter: dict, target_filter: dict, relationship: str
) -> dict[str, Any]:
    """
    ...
    src_ids, tgt_ids = _get_filtered_node_ids(
        source_filter, target_filter, is_admin, allowed_locations
    )
    all_ids = list(set(src_ids + tgt_ids))
    existing_rels = topology_repo.get_cis_relationship_summary(
        all_ids,
