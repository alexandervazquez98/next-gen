...
async def event_batch_pruner(
    ...
    #
    # fix-sse-pruner-streaming-blocking v1.14.5 attempted to use the
    # neo4j 5.16 AsyncDriver surface (``async with driver.session()``,
    # ``await session.run(...)``, ``async for record in result:``,
    # ``async with session.begin_transaction() as tx:``,
    # ``await tx.commit()``). That broke at runtime because
    # ``get_db()`` returns the SYNC driver — the v1.14.5 deploy hit
    # ``TypeError: 'Session' object does not support the asynchronous
    # context manager protocol`` on the first record of the SSE
    # endpoint and was rolled back in v1.14.5-revert.
    #
    ...
    while total_processed < total:
        ...
        try:
            # Run the sync PAGE query in a thread so the event loop stays free.
