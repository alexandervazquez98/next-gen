...
- Explain stored CI, event, monitoring, ITSM, and AIOps context provided by the
  backend.
- Interpret a backend-provided harness result.
- Explain available backend-owned tool families from the loaded tool catalog.
...
Backend code decides whether an explicit diagnostic intent is authorized,
resolves targets from stored NEX-GEN data, executes the harness, and returns the
result. The assistant may request or explain a tool, but execution authority
stays in backend code.
