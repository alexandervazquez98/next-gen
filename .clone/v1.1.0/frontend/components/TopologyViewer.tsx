...
const TopologyViewer: React.FC<TopologyViewerProps> = ({ rootId, nodes, links }) => {
    ...
    useEffect(() => {
        ...
        relevantNodes.set(rootId, { ...root, level: 0, type: 'Root', x: 500, y: 400, vx: 0, vy: 0, fixed: true });
        ...
        const tick = () => {
            ...
            const CENTER_K = 0.005; // Weak center gravity
            const DAMPING = 0.8;    // Velocity decay

            // Target Radius per Level (Ultra Compact concentric rings)
            const getRadius = (level: number) => {
                switch (level) {
                    case 0: return 0;   // Root (Center)
                    case 1: return 80;  // Metrics of Root (Inner Orbit)
                    case 2: return 160; // Dependents (Middle Orbit)
                    case 3: return 240; // Metrics of Dependents (Outer Orbit)
                    default: return 200;
                }
            };
            ...
            for (let i = 0; i < nodes.length; i++) {
                for (let j = i + 1; j < nodes.length; j++) {
                    ...
                    if (!a.fixed && !a.dragged) { a.vx += fx; a.vy += fy; }
                    if (!b.fixed && !b.dragged) { b.vx -= fx; b.vy -= fy; }
                }
            }
            ...
            links.forEach((link: any) => {
                ...
                if (!s.fixed && !s.dragged) { s.vx += fx; s.vy += fy; }
                if (!t.fixed && !t.dragged) { t.vx -= fx; t.vy -= fy; }
            });
        ...
        >
            <svg width="100%" height="100%" viewBox="0 0 1000 800">
                <g transform={`translate(${transform.x},${transform.y}) scale(${transform.k})`}>
                    <defs>
                        ...
                        <marker id="arrow-green" viewBox="0 -5 10 10" refX="22" refY="0" markerWidth="6" markerHeight="6" orient="auto">
                            <path d="M0,-5L10,0L0,5" fill="#10b981" />
                        </marker>
                    </defs>

                    {/* Ring Guides (Visual Only) */}
                    <circle cx="500" cy="400" r="160" fill="none" stroke="#ffffff" strokeOpacity="0.05" strokeDasharray="5,5" strokeWidth={1} />
                    <circle cx="500" cy="400" r="240" fill="none" stroke="#ffffff" strokeOpacity="0.02" strokeDasharray="5,5" strokeWidth={1} />
                    ...
                    {graphData.nodes.map((node, i) => {
                        let color = '#fff';
                        if (node.level === 0) color = '#ef4444';      // Root
                        else if (node.level === 1) color = '#10b981'; // Root Metric
                        else if (node.level === 3) color = '#f59e0b'; // Dependent Metric
                        ...
                        return (
                            ...
                            >
                                ...
                                <text y={-15} textAnchor="middle" fill="white" fontSize={12} fontWeight="bold" className="pointer-events-none drop-shadow-md select-none">
                                    {node.label || node.display_name || node.name || node.id}
                                </text>
                                <text y={15} textAnchor="middle" fill="rgba(255,255,255,0.6)" fontSize={10} className="pointer-events-none select-none">
                                    {node.level === 0 ? 'ROOT' : node.level === 1 ? 'METRIC' : node.level === 2 ? 'DEPENDENT' : 'METRIC'}
                                </text>
                            </g>
                        );
                    })}
            ...
            <div className="absolute top-4 left-4 space-y-2 pointer-events-none">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-white shadow-black drop-shadow-md">Downstream (Dependents)</span>
                </div>
                {/* <div className="flex items-center gap-2">
