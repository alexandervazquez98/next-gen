...
| AD-3 | Synthetic-breach gate queries `:HAS_METRIC` from Neo4j at the start of the cycle and passes the configured `(ci_id, metric_id)` set into the helper; injection fires only for configured pairs without a real sample this cycle | Skip the gate (rely on in-memory updates list); pass a callback for per-call lookup | The in-memory `updates` list is an INVERTED signal: a CI without the metric is *also* absent from `updates`, so using `updates` as the gate produces phantom injections for any CI that has `icmp.availability` configured but not `icmp.jitter`/`icmp.packet_loss`. The pre-fix gate was wrong; the corrected gate queries `:HAS_METRIC` once per cycle (single small MATCH against `MetricDef.id IN [jitter, packet_loss]`, ~5 ms) and stores the result in a `set[tuple[str, str]]` that the helper consults with an O(1) lookup. **REQ-SYNTHETIC-BREACH-SCOPE is now COMPLIANT** for both spec scenarios (metric not configured on CI; HAS_METRIC deleted mid-cycle). |
