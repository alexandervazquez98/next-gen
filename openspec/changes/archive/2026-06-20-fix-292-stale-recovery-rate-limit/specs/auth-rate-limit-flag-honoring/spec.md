# Auth Rate Limit Flag Honoring — Delta Specification (Historical)

> **Note**: This is the **delta** spec kept for archive audit trail. The **consolidated (source-of-truth)** spec lives at `openspec/specs/auth-rate-limit-flag-honoring/spec.md`. The consolidated spec must be readable without referencing this archive.
...
All four requirements below are **newly added** by this change. There are no MODIFIED or REMOVED requirements.
...
- Any frontend work.
- Router-wide "flag is source of truth" refactor (deferred per spec revision).