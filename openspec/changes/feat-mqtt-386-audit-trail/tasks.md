...
- [ ] 2.1 RED — `test_mqtt_mapping_service.py`: every action emits `event_type=MQTT_MAPPING_<ACT>` + `outcome=DENIED`; `record_denied` NEVER called. Spec: *Denied approve attempt is audited*.
- [ ] 2.2 GREEN — Add `_enforce_manage_with_audit(*, current_user, db, request, mapping_id, event_type)` in `mqtt_mapping_service.py`: on no perm emits denial row, then `require_mqtt_permission` raises.
