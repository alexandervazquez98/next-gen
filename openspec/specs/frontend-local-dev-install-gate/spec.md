...
- CI workflow edits (`frontend-ci.yml`, `smoke.yml`, `lint.yml`); handled by the parallel `ci-cd-pipeline` chain on `cicd/cd-lane`.
...
`openspec/changes/archive/2026-06-20-fix-298-sonner-install-gate/specs/frontend-local-dev-install-gate/spec.md`

Original PR: https://github.com/alexandervazquez98/next-gen/pull/305 (Closes #298).

Upstream context: `sonner` import was added in PR #291 (`fix(auth): local-only frontend idle logout with sonner toast (#287 PR2)`), commit `fb75972`. The install gate gap surfaced during the v1.13.2 patch release verification for #292.
