...
`openspec/changes/archive/2026-06-28-fix-collector-event-emission-cypher-rejection/`
during the SDD archive phase). The capability is new — there is no pre-existing
`cypher-param-fallback` root spec to merge with.

## Companion delta (deferred)

This change also carried a delta on the `event-deduplication` capability:
`openspec/changes/archive/2026-06-28-fix-collector-event-emission-cypher-rejection/specs/event-deduplication/spec.md`.

The `event-deduplication` root spec lives in the still-active
`openspec/changes/fix-event-duplication-cross-writer/` change. That change must
archive first so the canonical `openspec/specs/event-deduplication/spec.md`
exists. The delta is preserved in this archive as audit trail and will be
absorbed by the upstream change's archive phase; do not treat the delta in
this folder as already-merged into root specs.
