...
function Probe() {
  ...
  return (
    <div>
      <button onClick={() => void create.mutateAsync({
      ...
      <button onClick={() => void update.mutateAsync('map-1', { source_metric_name: 'x' }).catch(() => undefined)}>
        update
      </button>
      ...
      <button onClick={() => void thresholds.mutateAsync('map-1', { operator: '>', warning: 1, critical: 2 }).catch(() => undefined)}>
        thresholds
      </button>
    </div>
  );
...
describe('useMqttMutations — cache invalidation contract', () => {
  ...
  it.each([
  ...
  ] as const)('invalidates mqtt* keys after %s succeeds (no systemStatus)', async (button, _label, _endpoint) => {
    render(
    ...
    const calls = (client.invalidateQueries as unknown as { mock: { calls: unknown[][] } }).mock.calls;
    ...
    expect(invalidatedKeys).toContainEqual({ queryKey: ['mqtt', 'mappings', undefined, 'thresholds'] });
  ...
  it('surfaces a named-permission toast on 403 and leaves the cache untouched', async () => {
    ...
    await waitFor(() => {
      expect(sonnerMocks.toast.error).toHaveBeenCalledWith('Permission denied: MQTT_MAPPING_MANAGE');
    });
