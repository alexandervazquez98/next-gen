...
function link(
  id: string,
  medium: GraphLink["medium"] = "vpn",
  visible = true,
): GraphLink & { visible?: boolean } {
  return {
    id,
    source: `source-${id}`,
    target: `target-${id}`,
...
describe("buildVisibleTunnelHealthPlan", () => {
  ...
  it("suppresses failing links during the 2-minute cooldown and honors kill switches", () => {
    ...
    expect(
      buildVisibleTunnelHealthPlan([tunnel], {
        nowMs: 60_000,
        cooldownUntilByLinkId: { [encoded]: 120_000 },
      }),
    ).toMatchObject({
      linkIds: [],
    ...
    expect(
      buildVisibleTunnelHealthPlan([tunnel], { nowMs: 0, pollingDisabled: true }),
    ).toMatchObject({
      linkIds: [],
  ...
  it("uses deterministic 10-20% jitter and disables retry", () => {
    const plan = buildVisibleTunnelHealthPlan([link("jitter")], {
      nowMs: 0,
      jitterSeed: 0.5,
    });
  ...
  it("rotates through capped visible IDs instead of polling only the first four forever", () => {
    const plan = buildVisibleTunnelHealthPlan(
      Array.from({ length: 12 }, (_, index) => link(`vpn-${index}`)),
      { nowMs: 0 },
    );
...
describe("tunnel health scheduling", () => {
  ...
  it("does not re-enter cooldown from a production-cached error after cooldown expires", async () => {
    ...
    try {
      ...
      mockApiGet.mockRejectedValueOnce({ status: 500 }).mockReturnValueOnce(retryPromise);

      render(<HookProbe links={[link("production-cache-retry")]} />, {
