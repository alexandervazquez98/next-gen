...
export function buildVisibleTunnelHealthPlan(
  ...
  const pollingDisabled =
    options.pollingDisabled ?? (envPollingDisabled() || runtimePollingDisabled());
...
export function selectActiveTunnelHealthIds(linkIds: string[], cycle: number): string[] {
  ...
  return Array.from(
    { length: TUNNEL_HEALTH_LIMITS.maxInFlight },
    (_, offset) => linkIds[(start + offset) % linkIds.length],
  );
}
...
export function updateTunnelHealthCooldown(
  cooldown: CooldownMap,
  linkId: string,
  _errorKind: TunnelHealthErrorKind,
  nowMs: number,
): CooldownMap {
...
function pruneExpiredCooldowns(cooldown: CooldownMap, nowMs: number): CooldownMap {
  const activeEntries = Object.entries(cooldown).filter(
...
export function useVisibleTunnelHealth(links: VisibleLink[]) {
  ...
  const telemetryBatcherRef = useRef(createTunnelHealthTelemetryBatcher());
  const lastTelemetrySignature = useRef("");
  const accountedFailureAtByLinkIdRef = useRef<Record<string, number>>({});
  ...
  const linkById = useMemo(() => {
    const entries = links
      ...
      .map((link) => [encodeTunnelLinkId(link), link] as const);
    return Object.fromEntries(entries) as Record<string, VisibleLink>;
  }, [links]);
  const plan = useMemo(
    () => buildVisibleTunnelHealthPlan(links, { cooldownUntilByLinkId }),
    [links, cooldownUntilByLinkId],
  );
  const activeLinkIds = useMemo(
    () => selectActiveTunnelHealthIds(plan.linkIds, cycle),
    [plan.linkIds, cycle],
  );
  ...
  const { healthByLinkId, visualByLinkId, failureByKind, success, failedQueries } = useMemo(() => {
    ...
    activeLinkIds.forEach((linkId, index) => {
      ...
      if (query?.error) {
        ...
        const isNewFailure =
          errorUpdatedAt > 0 && accountedFailureAtByLinkIdRef.current[linkId] !== errorUpdatedAt;
        if (isNewFailure) {
          nextFailedQueries.push({ linkId, errorKind });
          nextFailureByKind[errorKind] = (nextFailureByKind[errorKind] ?? 0) + 1;
        }
        const staleHealth = queryClient.getQueryData<TunnelHealthResponse>(
          queryKeys.tunnelHealth(linkId),
        );
        nextVisualByLinkId[linkId] = resolveTunnelHealthFallbackVisual(
          linkById[linkId] ?? { medium: undefined },
          staleHealth,
          errorKind,
        );
      }
  ...
  const failedQueriesSignature = failedQueries
    .map(({ linkId, errorKind }) => `${linkId}:${errorKind}`)
    .join("|");
  ...
  useEffect(() => {
    ...
    setCooldownUntilByLinkId((current) => {
      const nowMs = Date.now();
      return currentFailedQueries.reduce((next, item) => {
        const queryState = queryClient.getQueryState<TunnelHealthResponse>(
          queryKeys.tunnelHealth(item.linkId),
        );
        if (queryState?.errorUpdatedAt) {
          accountedFailureAtByLinkIdRef.current[item.linkId] = queryState.errorUpdatedAt;
        }
        return updateTunnelHealthCooldown(next, item.linkId, item.errorKind, nowMs);
      }, current);
    });
  }, [failedQueriesSignature, queryClient]);

  useEffect(() => {
    const signature = JSON.stringify({
      scheduled: activeLinkIds.length,
      skipped: plan.skippedOverCap,
      suppressed: plan.suppressedCooldown,
      success,
      failureByKind,
      disabled: plan.pollingDisabled,
    });
    if (signature === lastTelemetrySignature.current) return;
    lastTelemetrySignature.current = signature;
    void telemetryBatcherRef.current.flush({
      scheduled: activeLinkIds.length,
    ...
    failureByKind,
    plan.pollingDisabled,
    plan.skippedOverCap,
    plan.suppressedCooldown,
    success,
  ]);
