  ...
  const groupedEvents = useMemo(() => {
    ...
    links.forEach((link) => {
      ...
      if (
        ...
        const providerRoots = Array.from(eventMap.values()).filter(
          (e) => e.ci_id === providerId && e.isRoot && getSeverityWeight(e.severity) >= 2,
        ); // Critical or Warning Provider

        const consumerRoots = Array.from(eventMap.values()).filter(
          (e) => e.ci_id === consumerId && e.isRoot,
        );
