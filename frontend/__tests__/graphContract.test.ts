...
describe("graphContract fixtures", () => {
    ...
    describe.each([
        ...
        it("never carries any forbidden sensitive field (REQ-9)", () => {
            const leaked = OVERVIEW_FORBIDDEN_FIELDS.filter((k) =>
                k in (data as unknown as Record<string, unknown>),
            );
            expect(leaked).toEqual([]);
            for (const c of data.clusters) {
                const clusterLeaks = OVERVIEW_FORBIDDEN_FIELDS.filter((k) =>
                    k in (c as unknown as Record<string, unknown>),
                );
        ...
        it("safe_geo_precision is one of the allowed enum values", () => {
            expect(SAFE_GEO_PRECISION_VALUES).toContain(
                data.aggregate_policy.safe_geo_precision,
            );
        });
    });
    ...
    describe.each([
        ...
        it("sensitive_source is one of the allowed enum values (REQ-9)", () => {
            expect(SENSITIVE_SOURCE_VALUES).toContain(
                data.projection_flags.sensitive_source,
            );
        });
        ...
        it("never exposes public_ip or metadata on a detail node (REQ-9)", () => {
            for (const n of data.nodes) {
                const leaked = DETAIL_NODE_FORBIDDEN_FIELDS.filter((k) =>
                    k in (n as unknown as Record<string, unknown>),
                );
                expect(leaked).toEqual([]);
            }
        });
...
describe("graphContract URL builders", () => {
    ...
    it("buildDetailUrl accepts the unassigned sentinel and a normal cluster_id", () => {
        ...
        expect(buildDetailUrl("location:dc-1")).toBe(
            "/api/graph/detail/location%3Adc-1",
        );
    });
...
describe("graphContract enum constants", () => {
    it("exposes exactly the spec-pinned SafeGeoPrecision values", () => {
        expect([...SAFE_GEO_PRECISION_VALUES].sort()).toEqual([
            "city",
            "none",
            "region",
        ]);
