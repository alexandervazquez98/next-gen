                ...
                <div className="p-8 border-b border-white/5 bg-white/5 relative overflow-hidden">
                    <div className="relative z-10">
                        <div className="flex items-center gap-3 mb-2">
                            <span className={`w-3 h-3 rounded-full ${node.status === 'EXCEPTION' ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`}></span>
                            <span className="text-xs font-bold text-neutral-400 uppercase tracking-widest">{node.status} STATUS</span>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            >
                                <div className="flex justify-between items-start">
                                    <div className="flex flex-col">
                                        <span className="text-[10px] font-black uppercase opacity-60 tracking-wider mb-0.5">{metric.protocol}</span>
                                        <h4 className="font-black text-sm uppercase text-white/90">{metric.name}</h4>
                                    </div>
                                    {metric.status === 'CRITICAL' && <span className="material-symbols-outlined text-red-500 animate-pulse">warning</span>}
                                    {metric.status === 'WARNING' && <span className="material-symbols-outlined text-orange-500">priority_high</span>}
                                    {metric.status === 'OK' && <span className="material-symbols-outlined text-emerald-500">check_circle</span>}
                                </div>

                                <div className="flex items-end gap-2 py-1">
                                    <span className="text-3xl font-black tracking-tighter text-white">{metric.value ?? '--'}</span>
                                ...
                                <div className="mt-auto border-t border-white/5 pt-3 flex justify-between items-center text-[10px] font-mono opacity-50">
                                    ...
                                    <span>{metric.last_updated ? new Date(metric.last_updated).toLocaleTimeString() : 'NEVER'}</span>
                                </div>
                            </div>
                        ))}
                        ...
                        {(!node.metrics || node.metrics.length === 0) && (
                            <div className="col-span-full border border-dashed border-white/10 rounded-xl p-12 text-center text-neutral-500 flex flex-col items-center justify-center">
                                <span className="material-symbols-outlined text-4xl mb-3 opacity-50">sensor_window</span>
                                <p className="text-sm font-bold uppercase">No Telemetry Configured</p>
                                <p className="text-xs mt-1 opacity-60 max-w-xs">Assign metrics to this CI in the Inventory or Administration settings.</p>
                            </div>
                        )}
                    </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {Object.entries(node.metadata).map(([key, value]) => (
                                    <div key={key} className="bg-white/5 rounded-lg p-3 border border-white/5">
                                        <p className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider mb-1">{key}</p>
                                        <p className="text-sm font-mono text-neutral-300 truncate" title={String(value)}>{String(value)}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
