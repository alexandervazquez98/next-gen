...
import { useMapClustering, type Cluster } from "../hooks/useMapClustering";
...
export function buildLinkConfig(
	...
	const isWarning =
		!isCritical && (Boolean(source.hasWarning) || Boolean(target.hasWarning));
...
export function getNodeRenderConfig(node: {
	...
	const critCount =
		node.events?.filter((e) => e.severity === "CRITICAL").length ?? 0;
	const warnCount =
		node.events?.filter((e) => e.severity === "WARNING").length ?? 0;
	...
	useEffect(() => {
		if (nodes.length > 0 && !hasZoomed.current) {
			const validNodes = nodes.filter(
				(n) =>
					n.location &&
					Number.isFinite(n.location.lat) &&
					Number.isFinite(n.location.long),
			);
			if (validNodes.length > 0) {
				const bounds = L.latLngBounds(
					validNodes.map((n) => [n.location!.lat, n.location!.long]),
				);
		...
		>
			<div className="p-3 space-y-2">
				...
				<div className="text-xs font-bold text-white border-b border-white/10 pb-2">
					<span className="material-symbols-outlined text-brand-400 text-sm align-middle mr-1">
						location_on
					</span>
					{cluster.label}
				</div>
				...
				<div className="space-y-1.5 max-h-80 overflow-y-auto custom-scrollbar pr-1">
					{cluster.members.map((m) => {
						...
						return (
							...
							>
								<span
									className="text-[11px] text-neutral-300 truncate flex-1"
									title={m.node.label}
								>
									{m.node.label}
								</span>
								<span
									className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded shrink-0 ${statusColors[status]}`}
								>
									{status}
								</span>
							</div>
...
const ClusterMarker: React.FC<ClusterMarkerProps> = ({ cluster, onExpand }) => {
	...
	const hasAura =
		cluster.worstSeverity === "CRITICAL" || cluster.worstSeverity === "WARNING";
	...
	const clusterColor =
		SEVERITY_COLORS[cluster.worstSeverity] || SEVERITY_COLORS.OK;
	...
	const validLocations = cluster.members
		...
		.filter(
			(loc): loc is { lat: number; long: number } =>
				loc != null && Number.isFinite(loc.lat) && Number.isFinite(loc.long),
		);

	if (validLocations.length === 0) {
		console.warn(
			"[ClusterMarker] Skipping - no valid locations:",
			cluster.id,
			cluster.label,
		);
		return null;
	...
	const [firstLat, firstLong] =
		validLocations.length === 1
			...
			: [
					...
					validLocations.reduce((s, l) => s + l.long, 0) /
						...
						<div className="space-y-1">
							{cluster.members.map((m) => {
								...
								if (
									...
									!Number.isFinite(loc.long)
								)
									return null;
								return (
									<div
										key={m.node.id}
										className={`text-xs p-1 rounded ${getSeverityBg(m.events)}`}
									>
										{m.node.label} -{" "}
										{m.events.length > 0 ? m.events[0].severity : "OK"}
									</div>
								);
							})}
						</div>
...
const MapFocusZone: React.FC<MapFocusZoneProps> = ({
	cluster,
	nodesWithEvents,
	links,
	onReset,
	...
	useEffect(() => {
		if (cluster.members.length > 0) {
			...
			if (validLocations.length > 0) {
				...
				const bounds = L.latLngBounds(
					validLocations.map((loc) => [loc.lat, loc.long]),
				);
	...
	return (
		<>
								...
								{member.events.length > 0 ? (
									<div className="space-y-1">
										{member.events.map((e: any) => (
											<div
												key={e.id}
												className={`text-xs p-1 rounded ${e.severity === "CRITICAL" ? "bg-red-100 text-red-800" : "bg-yellow-100 text-yellow-800"}`}
											>
												{e.message}
											</div>
										))}
			...
			{links.map((link, i) => {
				...
				const sourceInCluster = cluster.members.some(
					(m) => m.node.id === link.source,
				);
				const targetInCluster = cluster.members.some(
					(m) => m.node.id === link.target,
				);

				if (!sourceNode || !targetNode || !sourceInCluster || !targetInCluster)
					return null;
				if (!sourceNode.location || !targetNode.location) return null;
				if (
					!Number.isFinite(sourceNode.location.lat) ||
					!Number.isFinite(sourceNode.location.long)
				)
					return null;
				if (
					!Number.isFinite(targetNode.location.lat) ||
					!Number.isFinite(targetNode.location.long)
				)
					return null;
...
class GeoViewErrorBoundary extends React.Component<
...
const MonitoringConsole: React.FC = () => {
	...
	const canForceCloseEvent =
		canCloseEvent && hasPermission("EVENT_FORCED_CLOSE");
	const canRunDiagnostics = hasPermission("RUN_DIAGNOSTICS");
	const canOpenEventModal =
		canViewEventDetail || canAckEvent || canCloseEvent || canRunDiagnostics;
	const eventDetailQuery = useEventDetailQuery(
		selectedEventId,
		commentModalOpen && canViewEventDetail,
	);
	...
	const {
		...
		expandCluster,
		collapseCluster,
	} = useMapClustering(nodesWithEvents, events);
	...
	const resetMapView = useCallback(() => {
		...
		const validNodes = nodesWithEvents.filter(
			(n) =>
				n.location &&
				Number.isFinite(n.location.lat) &&
				Number.isFinite(n.location.long),
	...
	const kpiCritical = openEvents.filter(
	...
	const cleanableCount = events.filter(
		(e) => e.status === "RECOVERED" && !e.ack,
	).length;

	// Streaming prune state
	const pruneState = eventMutations.usePruneRecovered();
	const [, setPruneError] = React.useState<string | null>(null);

	// Completion feedback for prune operation
	React.useEffect(() => {
		if (pruneState.isComplete && pruneState.progress) {
			alert(
				`Limpieza completada: ${pruneState.progress.processed} eventos cerrados`,
			);
		}
	...
	const sortedEventStream = useMemo(
		() =>
			[...groupedEvents].sort((a, b) => {
				const statusDelta =
					(STREAM_STATUS_WEIGHTS[b.status] ?? 0) -
					(STREAM_STATUS_WEIGHTS[a.status] ?? 0);
	...
	const filteredEventStream = useMemo(() => {
		if (streamFilter === "CRITICAL")
			return sortedEventStream.filter(
				(e) => e.status === "OPEN" && e.severity === "CRITICAL",
			);
		if (streamFilter === "WARNING")
			return sortedEventStream.filter(
				(e) => e.status === "OPEN" && e.severity === "WARNING",
			);
		if (streamFilter === "ACK")
			return sortedEventStream.filter((e) => e.status === "ACK");
		...
		<div className="h-full flex flex-col bg-surface-950 overflow-hidden relative">
			...
			<div className="h-16 px-8 flex items-center justify-between border-b border-white/5 glass z-10">
				<div className="flex items-center gap-4">
					<h2 className="text-xl font-black text-white uppercase tracking-tighter flex items-center gap-2">
						<span className="material-symbols-outlined text-brand-400">
							notifications_active
						</span>
						Event Console
						{isSmartMode &&
							events.length >= SMART_CULL_THRESHOLD &&
							viewMode === "DASHBOARD" && (
					...
					<div className="flex bg-black/20 p-1 rounded-lg border border-white/5">
						<button
							...
							className={`px-4 py-1.5 rounded-md text-xs font-bold uppercase transition-all ${viewMode === "DASHBOARD" ? "bg-brand-600 text-white shadow-lg" : "text-neutral-500 hover:text-white"}`}
						>
							Stream
						</button>
						<button
							onClick={() => setViewMode("MAP")}
							className={`px-4 py-1.5 rounded-md text-xs font-bold uppercase transition-all ${viewMode === "MAP" ? "bg-brand-600 text-white shadow-lg" : "text-neutral-500 hover:text-white"}`}
						>
							Geo View
						</button>
					</div>
					...
					<div className="h-full p-8 overflow-y-auto custom-scrollbar space-y-8">
						...
						<div className="grid grid-cols-1 md:grid-cols-4 gap-6">
							<StatCard
								...
								color="text-red-500"
								bg="bg-red-500/10"
								animate={kpiCritical > 0}
								active={streamFilter === "CRITICAL"}
							...
							<StatCard
								...
								bg="bg-yellow-500/10"
								active={streamFilter === "WARNING"}
								onClick={() => setStreamFilter("WARNING")}
							/>
							<StatCard
								label="Acknowledged"
								value={kpiAck}
								...
								color="text-blue-400"
								bg="bg-blue-500/10"
								active={streamFilter === "ACK"}
								onClick={() => setStreamFilter("ACK")}
							/>
							<StatCard
								label="Total Active"
								value={events.length}
									...
									<tbody className="text-sm divide-y divide-white/5">
										{filteredEventStream.length === 0 ? (
											<tr>
												<td
													colSpan={7}
													className="p-8 text-center text-neutral-600 italic"
												>
													No active alarms. System healthy.
												</td>
											</tr>
										) : (
					...
					<GeoViewErrorBoundary>
						<div className="h-full w-full relative">
							...
							>
								...
								{!expandedClusterId &&
									links.map((link, i) => {
										const source = nodesWithEvents.find(
											(n) => n.id === link.source,
										);
										const target = nodesWithEvents.find(
											(n) => n.id === link.target,
										);
										...
										if (
											source?.location?.lat &&
											target?.location?.lat &&
											Number.isFinite(source.location.lat) &&
											Number.isFinite(source.location.long) &&
											Number.isFinite(target.location.lat) &&
											Number.isFinite(target.location.long)
										) {
											const positions: [number, number][] = [
			...
			{commentModalOpen &&
				...
				(() => {
					...
					const isBusinessContextReady =
						canViewEventDetail &&
						eventDetailQuery.isSuccess &&
						Boolean(eventDetail);
					...
					const site = isBusinessContextReady
						? (businessContext?.site ??
							detailEvent?.ci_ref?.location_name ??
							displayEvent.ci_location_name ??
							node?.location_name ??
							null)
						: (detailEvent?.ci_ref?.location_name ??
							displayEvent.ci_location_name ??
							node?.location_name ??
							null);
					const businessServiceName = isBusinessContextReady
						? (businessContext?.business_service?.name ?? null)
						: null;
					const impactedUsers = isBusinessContextReady
						? (businessContext?.impacted_users ?? null)
						: null;
					const category =
						businessContext?.service_catalog?.category ??
						node?.category ??
						node?.type ??
						null;
					const ageMs =
					...
					const parseTimelineEntry = (raw: string) => {
						...
						const forcedAuditMatch = raw.match(
							/^\[AUDIT\]\[FORCED_CLOSE\]\s+([\s\S]+?)(?:\s\((.+)\))?\s*$/,
						);
						if (forcedAuditMatch)
							return {
								type: "force_close" as const,
								user: extractAuditActor(forcedAuditMatch[1]) || "",
								body: forcedAuditMatch[1],
								ts: forcedAuditMatch[2],
								raw,
							};
						const closeAuditMatch = raw.match(
							/^\[AUDIT]\[CLOSE\]\s+([\s\S]+?)(?:\s\((.+)\))?\s*$/,
						);
						if (closeAuditMatch)
							return {
								type: "close" as const,
								user: extractAuditActor(closeAuditMatch[1]) || "",
								body: closeAuditMatch[1],
								ts: closeAuditMatch[2],
								raw,
							};
						const ownershipAuditMatch = raw.match(
							/^\[AUDIT\]\[OWNERSHIP\]\s+([\s\S]+?)(?:\s\((.+)\))?\s*$/,
						);
						if (ownershipAuditMatch)
							return {
								type: "ownership" as const,
								user: extractAuditActor(ownershipAuditMatch[1]) || "",
								body: ownershipAuditMatch[1],
								ts: ownershipAuditMatch[2],
								raw,
							};
						const forceMatch = raw.match(/^\[CIERRE FORZADO/);
						if (forceMatch)
							return { type: "force_close" as const, user: "", body: raw, raw };
						const closeMatch = raw.match(/^\[CIERRE/);
						if (closeMatch)
							return { type: "close" as const, user: "", body: raw, raw };
						const stdMatch = raw.match(/^(.+?):\s([\s\S]+?)\s\((.+)\)$/);
						if (stdMatch) {
							const [, user, body, ts] = stdMatch;
							const structuredOwnershipMatch = body.match(
								/^\[AUDIT\]\[OWNERSHIP\]\s+([\s\S]+)$/m,
							);
							if (structuredOwnershipMatch)
								return {
									type: "ownership" as const,
									user,
									body: structuredOwnershipMatch[1],
									ts,
									raw,
								};
							const structuredForceCloseMatch = body.match(
								/^\[AUDIT\]\[FORCED_CLOSE\]\s+([\s\S]+)$/m,
							);
							if (structuredForceCloseMatch)
								return {
									type: "force_close" as const,
									user,
									body: structuredForceCloseMatch[1],
									ts,
									raw,
								};
							const structuredCloseMatch = body.match(
								/^\[AUDIT\]\[CLOSE\]\s+([\s\S]+)$/m,
							);
							if (structuredCloseMatch)
								return {
									type: "close" as const,
									user,
									body: structuredCloseMatch[1],
									ts,
									raw,
								};
							const ownershipBodyMatch = body.match(/^\[OWNERSHIP\] (.+)$/m);
							if (ownershipBodyMatch)
								return {
									type: "ownership" as const,
									user,
									body: ownershipBodyMatch[1],
									ts,
									raw,
								};
							if (/^\[CIERRE FORZADO/.test(body))
								return { type: "force_close" as const, user, body, ts, raw };
							if (/^\[CIERRE/.test(body))
								return { type: "close" as const, user, body, ts, raw };
					...
					const timelineEntries = (detailEvent?.comments ?? []).map(
						parseTimelineEntry,
					);
							...
							<div className="bg-[#0f1117] border border-white/10 rounded-2xl w-full max-w-[96%] 2xl:max-w-[88%] h-full max-h-[92vh] shadow-2xl flex flex-col overflow-hidden">
								...
								<div className="flex-shrink-0 border-b border-white/10 bg-black/30">
									...
									<div className="px-6 pt-5 pb-3 flex justify-between items-start gap-4">
										<div className="flex-1 min-w-0">
											<div className="flex items-center gap-3 flex-wrap">
												<span
													className={`px-3 py-1 rounded text-xs font-black uppercase tracking-wider flex-shrink-0 ${displayEvent.severity === "CRITICAL" ? "bg-red-500 text-white" : displayEvent.severity === "WARNING" ? "bg-yellow-400 text-black" : "bg-blue-500 text-white"}`}
												>
											...
											<div className="mt-2 text-neutral-400 text-xs flex flex-wrap gap-x-4 gap-y-1">
												<span>
													...
													{detailEvent?.ci_ref?.id ||
													displayEvent.ci_node_id ? (
												...
												<span>
													...
													{detailEvent?.ci_ref?.label ||
													displayEvent.ci_name ? (
													...
													{(detailEvent?.ci_ref?.hostname ||
														displayEvent.ci_hostname) && (
												...
												<span>
													<strong className="text-neutral-300">Métrica:</strong>{" "}
													{displayEvent.metric_name || "—"}
												</span>
												<span>
													<strong className="text-neutral-300">
														Protocolo:
													</strong>{" "}
													{displayEvent.metric_protocol || "N/A"}
												</span>
												<span>
										...
										<button
											...
											className="text-neutral-500 hover:text-white transition-colors flex-shrink-0 mt-1"
										>
									...
									<div className="px-6 pb-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
										...
										<div
											className={`rounded-lg px-3 py-2 text-xs ${slaCritical ? "bg-red-500/20 border border-red-500/40" : "bg-white/5"}`}
										>
											<div
												className={`uppercase font-bold tracking-wider mb-0.5 ${slaCritical ? "text-red-400" : "text-neutral-500"}`}
											>
												SLA Restante
											</div>
											<div
												className={`font-black text-sm ${slaCritical ? "text-red-400" : "text-white"}`}
											>
												{slaRemaining !== null ? (
													slaCritical ? (
														`⚠ ${slaRemaining} min`
									...
									<div className="px-6 pb-3 text-[10px] uppercase tracking-widest text-neutral-500">
										Contexto de negocio:{" "}
										{businessContextStatus === "loading"
											? "cargando"
											: businessContextStatus === "degraded"
												? "degradado"
												: businessContextStatus === "summary-only"
													? "resumen local"
									...
									{/* ── M2: Ownership Bar ── */}
									<div
										className={`px-6 py-2 border-t border-white/5 flex flex-wrap items-center gap-4 text-xs ${!isAssigned ? "bg-red-950/30" : "bg-black/20"}`}
									>
										{/* Assigned to */}
											...
											{isAssigned ? (
												...
												<span className="flex items-center gap-2">
													...
													{canAckEvent && (
														<button
															onClick={() =>
																void handleTakeCase(displayEvent.id)
															}
															disabled={isTakingCase}
															className={`px-2 py-0.5 rounded font-bold uppercase tracking-wide transition-colors ${
																isTakingCase
																	? "bg-emerald-900 text-white/70 cursor-wait"
																	: "bg-emerald-600 hover:bg-emerald-500 text-white"
															}`}
														>
															{isTakingCase ? "Tomando..." : "Tomar caso"}
														</button>
													)}
													{takeCaseError && (
										...
										<div className="flex items-center gap-1.5">
											...
											<span className="px-2 py-0.5 bg-brand-600/30 border border-brand-500/40 rounded text-brand-300 font-black">
												{eventTier ?? "No definido"}
											</span>
										</div>
										<div className="w-px h-4 bg-white/10" />
										{/* Estado */}
									...
									<>
										...
										{canViewEventDetail &&
											...
											!eventDetail && (
												<div className="mx-5 mt-5 rounded-lg border border-red-500/20 bg-red-950/20 px-4 py-3 text-xs text-red-200">
													No se pudo cargar el detalle protegido. Seguís
													trabajando con el resumen del stream.
												</div>
											)}
										{detailFetchBlocked && (
											<div className="mx-5 mt-5 rounded-lg border border-white/10 bg-white/5 px-4 py-3 text-xs text-neutral-300">
												Sin permiso `EVENT_VIEW`: se muestra el resumen local y
												solo las acciones autorizadas.
											</div>
										)}
										{/* Left: M3 Timeline */}
										<div className="w-[270px] lg:w-[310px] xl:w-[360px] flex-shrink-0 flex flex-col border-r border-white/10 bg-[#0f1117]">
											<div className="px-5 pt-5 pb-3 flex-shrink-0 border-b border-white/5">
												<h4 className="text-xs font-black text-neutral-400 uppercase tracking-widest flex items-center gap-2">
													<span className="material-symbols-outlined text-brand-400 text-sm">
														history
													</span>
													Timeline de Investigación
												</h4>
												<p className="text-[10px] text-neutral-600 mt-1">
													Registro append-only · fuente de auditoría
												</p>
											</div>
											<div className="flex-1 overflow-y-auto custom-scrollbar px-5 py-4 space-y-3">
												...
												{timelineEntries.map((entry, i) => {
													...
													return (
														<div
															key={i}
															className={`relative pl-6 border-l-2 ${color.includes("emerald") ? "border-emerald-500/30" : color.includes("neutral") ? "border-neutral-500/30" : "border-brand-500/30"} pb-3`}
														>
															<div
																className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-[#0f1117] border-2 ${color.includes("emerald") ? "border-emerald-500" : color.includes("neutral") ? "border-neutral-500" : "border-brand-500"} flex items-center justify-center`}
															>
																<span
																	className={`material-symbols-outlined text-[8px] ${color.split(" ")[1]}`}
																>
																	{icon}
																</span>
															</div>
															<div
																className={`text-[10px] font-mono mb-1 ${color.includes("emerald") ? "text-emerald-600" : color.includes("neutral") ? "text-neutral-600" : "text-neutral-600"}`}
															>
																{entry.ts
																...
																{entry.type === "diagnostic"
																	...
																	: entry.type === "ownership"
																		...
																		: entry.type === "close"
																			? "Evento cerrado"
																			: entry.type === "force_close"
																				? "Cierre forzado"
																				: "Nota de investigación"}
															</div>
															<div
																className={`p-2.5 rounded-lg text-xs leading-relaxed whitespace-pre-wrap font-mono border ${
																	entry.type === "diagnostic"
																		? "bg-brand-950/30 border-brand-500/15 text-brand-200"
																		: entry.type === "ownership"
																			? "bg-emerald-950/30 border-emerald-500/15 text-emerald-200"
																			: entry.type === "close" ||
																					entry.type === "force_close"
																				? "bg-neutral-900/50 border-neutral-700/30 text-neutral-300"
											...
											{canAckEvent && (
												<div className="flex-shrink-0 border-t border-white/5 px-5 py-4">
													<h4 className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-2">
														Agregar nota
													</h4>
													<textarea
										...
										<div className="flex-1 flex flex-col min-w-0 bg-black/20 border-r border-white/10">
											...
											<div className="flex-shrink-0 px-5 pb-3 text-[10px] text-neutral-600 text-center italic">
												Blast radius visualizado hasta 3 niveles de dependencia
												directa
											</div>
										</div>
										...
										<div className="w-[260px] lg:w-[300px] xl:w-[330px] flex-shrink-0 flex flex-col bg-[#0f1117] overflow-y-auto custom-scrollbar">
											...
											<div className="px-5 pt-5 pb-3 border-b border-white/5">
												<RelatedAlarmsPanel
													...
													enabled={canViewEventDetail}
												/>
											</div>

											{/* Quick actions */}
											<div className="px-5 py-4 border-b border-white/5">
												<h4 className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-3">
													Acciones rápidas
												</h4>
														...
														(!closeFlowOpen ? (
															...
															<div className="bg-neutral-900/80 border border-white/10 rounded-xl p-4 space-y-3">
																<div className="flex items-center justify-between">
																	...
																	<button
																		onClick={() => setCloseFlowOpen(false)}
																...
																{!closeForcedMode ? (
																	<>
																		...
																		<div>
																			...
																			<select
																				...
																				onChange={(e) =>
																					setCloseRootCause(e.target.value)
																				}
																			...
																			>
																				...
																				<option value="Falla de hardware">
																					Falla de hardware
																				</option>
																				<option value="Error de configuración">
																					Error de configuración
																				</option>
																				<option value="Problema de capacidad / recursos">
																					Problema de capacidad / recursos
																				</option>
																				<option value="Falla de proveedor / enlace externo">
																					Falla de proveedor / enlace externo
																				</option>
																				<option value="Causa desconocida">
																					Causa desconocida (requiere nota)
																				</option>
																			</select>
																		</div>
																		...
																		<div>
																			...
																			/>
																			<div
																				className={`text-[10px] mt-0.5 text-right ${closeNote.length < 20 ? "text-neutral-600" : "text-emerald-500"}`}
																			>
																	...
																	<>
																		<div className="text-[10px] text-red-400 font-bold uppercase bg-red-950/30 border border-red-500/20 rounded px-2 py-1">
																			⚠ Cierre forzado — quedará registrado en
																			el timeline
																		</div>
																		<textarea
																			value={closeForcedReason}
																			onChange={(e) =>
																				setCloseForcedReason(e.target.value)
																			}
																		...
																		<div className="flex gap-2">
																			<button
																				...
																				className="flex-1 py-2 bg-neutral-800 text-neutral-400 rounded-lg text-xs font-bold"
																			>
																				Cancelar
																			</button>
																			<button
											...
											{canRunDiagnostics && (
												<div className="px-5 py-4">
													<h4 className="text-[10px] font-black text-brand-400 uppercase tracking-widest mb-3">
													...
													<div className="bg-black/40 rounded-xl border border-white/5 p-3">
														...
														<button
															onClick={async () => {
																...
																try {
																	await eventMutations.diagnoseEvent(
																		selectedEventId,
																		{ user: CURRENT_USER },
																	);
																} catch (e) {
														...
														>
															<span
																className={`material-symbols-outlined text-sm ${isDiagnosing ? "animate-spin" : ""}`}
															>
														...
														<p className="text-[10px] text-neutral-600 mt-2 leading-relaxed">
															El resultado se registrará automáticamente en el
															timeline.
														</p>
													</div>
												</div>
...
const StatCard = ({
	...
	>
		...
		<div
			className={`w-12 h-12 rounded-xl flex items-center justify-center ${color} bg-black/20`}
		>
...
const RelatedAlarmsPanel = ({
	...
	return (
		<RelatedAlarmsPanelWithQuery ciId={ciId} currentEventId={currentEventId} />
	);
};

const RelatedAlarmsPanelWithQuery = ({
	ciId,
	currentEventId,
}: {
	ciId: string;
		...
		<div className="mb-2">
			...
			<div className="space-y-2">
				{displayed.map((evt) => (
					<div
						key={evt.id}
						className="bg-surface-950 p-2 rounded-lg border border-white/5 flex gap-3 text-xs items-center hover:bg-white/5 transition-colors cursor-default"
					>
						<div
							className={`w-8 h-8 rounded-md flex items-center justify-center flex-shrink-0 ${evt.severity === "CRITICAL" ? "bg-red-500/10 text-red-500" : "bg-yellow-500/10 text-yellow-500"}`}
						>
							<span className="material-symbols-outlined text-lg">
						...
						<div className="flex-1 min-w-0">
							<div className="font-bold text-white mb-0.5 truncate">
								{evt.metric_name || "System Alert"}
							</div>
							<div className="text-neutral-400 leading-tight truncate">
								{evt.message}
							</div>
						</div>
