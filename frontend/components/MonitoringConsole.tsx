...
const MapInstanceCapture = ({ onReady }: { onReady: (_map: L.Map) => void }) => {
  const map = useMap();
  useEffect(() => {
    onReady(map);
  }, [map, onReady]);
  ...
  useEffect(() => {
    if (nodes.length > 0 && !hasZoomed.current) {
      const validNodes = nodes.filter(
        (n) => n.location && Number.isFinite(n.location.lat) && Number.isFinite(n.location.long),
      );
  ...
  const validLocations = cluster.members
    ...
    .filter(
      (loc): loc is { lat: number; long: number } =>
        loc !== null && loc !== undefined && Number.isFinite(loc.lat) && Number.isFinite(loc.long),
    );
  ...
  return (
    <>
      ...
      {links.map((link, i) => {
        ...
        if (!Number.isFinite(sourceNode.location.lat) || !Number.isFinite(sourceNode.location.long))
          return null;
        if (!Number.isFinite(targetNode.location.lat) || !Number.isFinite(targetNode.location.long))
          return null;
        ...
        return (
          <React.Fragment key={`expanded-link-${i}`}>
            <AnimatedPolyline
              ...
              animationConfig={
                cfg.animate ? { from: cfg.animFrom, to: cfg.animTo, dur: cfg.animDur } : null
              }
            />
...
const MonitoringConsole: React.FC = () => {
  ...
  const rootEvents = events.filter(
    (e) => (e.correlation_type ?? "ROOT") === "ROOT",
  );
  ...
  const totalAffectedCIs = rootEvents.reduce(
    (sum, e) => sum + (e.affected_count ?? 0),
    0,
  );
  ...
  const sortedEventStream = useMemo(
    () =>
      [...groupedEvents].sort((a, b) => {
        const statusDelta =
          (STREAM_STATUS_WEIGHTS[b.status] ?? 0) - (STREAM_STATUS_WEIGHTS[a.status] ?? 0);
        ...
        const ageDelta = (getEventOpenAgeMs(b) ?? 0) - (getEventOpenAgeMs(a) ?? 0);
        if (ageDelta !== 0) return ageDelta;

        return (parseEventTimestamp(b.created_at) ?? 0) - (parseEventTimestamp(a.created_at) ?? 0);
      }),
    [groupedEvents],
  );
  const filteredEventStream = useMemo(() => {
    if (streamFilter === "CRITICAL")
      return sortedEventStream.filter((e) => e.status === "OPEN" && e.severity === "CRITICAL");
    if (streamFilter === "WARNING")
      return sortedEventStream.filter((e) => e.status === "OPEN" && e.severity === "WARNING");
          ...
          <div className="h-full p-8 overflow-y-auto custom-scrollbar space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <StatCard
                ...
                subLabel={
                  totalAffectedCIs > 0
                    ? `affecting ${totalAffectedCIs} CIs`
                    : null
                }
              />
            </div>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="text-sm divide-y divide-white/5">
                    {filteredEventStream.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="p-8 text-center text-neutral-600 italic">
      ...
      {commentModalOpen &&
        ...
        (() => {
          ...
          const isBusinessContextReady =
            canViewEventDetail && eventDetailQuery.isSuccess && Boolean(eventDetail);
          ...
          const impactedUsers = isBusinessContextReady
            ...
            : null;
          const category =
            businessContext?.service_catalog?.category ?? node?.category ?? node?.type ?? null;
                ...
                <div className="flex-shrink-0 border-b border-white/10 bg-black/30">
                  ...
                  <div className="px-6 pt-5 pb-3 flex justify-between items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 text-neutral-400 text-xs flex flex-wrap gap-x-4 gap-y-1">
                        ...
                        <span>
                          <strong className="text-neutral-300">Protocolo:</strong>{" "}
                  ...
                  <div className="px-6 pb-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                    ...
                    <div className="bg-white/5 rounded-lg px-3 py-2 text-xs">
                      ...
                      <div className="text-white font-semibold">
                        {site || <span className="text-neutral-600 italic">No configurado</span>}
                      </div>
                    </div>
                    <div className="bg-white/5 rounded-lg px-3 py-2 text-xs">
                      <div className="text-neutral-500 uppercase font-bold tracking-wider mb-0.5">
                  ...
                  <>
                    ...
                    {canViewEventDetail && eventDetailQuery.isError && !eventDetail && (
                      <div className="mx-5 mt-5 rounded-lg border border-red-500/20 bg-red-950/20 px-4 py-3 text-xs text-red-200">
                        No se pudo cargar el detalle protegido. Seguís trabajando con el resumen del
                        stream.
                      </div>
                    )}
                    {detailFetchBlocked && (
                      <div className="mx-5 mt-5 rounded-lg border border-white/10 bg-white/5 px-4 py-3 text-xs text-neutral-300">
                        Sin permiso `EVENT_VIEW`: se muestra el resumen local y solo las acciones
                        autorizadas.
                      </div>
                    )}
                    {/* Left: M3 Timeline */}
                      ...
                      <div className="flex-1 overflow-y-auto custom-scrollbar px-5 py-4 space-y-3">
                        ...
                        <div className="relative pl-6 border-l-2 border-red-500/40 pb-3">
                          ...
                          <div className="text-[10px] text-neutral-600 font-mono mb-1">
                            {formatEventTime(displayEvent.created_at)} · Sistema · Evento disparador
                          </div>
                          <div className="bg-red-950/30 border border-red-500/20 p-2.5 rounded-lg text-xs text-red-200 leading-relaxed">
                            <span className="font-black text-red-400">DISPARADOR:</span>{" "}
                            {displayEvent.message}
                          </div>
                        </div>
                                ...
                                className={`p-2.5 rounded-lg text-xs leading-relaxed whitespace-pre-wrap font-mono border ${
                                  entry.type === "diagnostic"
                                    ...
                                    : entry.type === "ownership"
                                      ...
                                      : entry.type === "close" || entry.type === "force_close"
                                ...
                                {!closeForcedMode ? (
                                  <>
                                    ...
                                    <div>
                                      <label className="text-[10px] font-bold text-neutral-500 uppercase block mb-1">
                                        2. Nota de cierre{" "}
                                        <span className="text-neutral-600">(mín. 20 chars)</span>{" "}
                                  ...
                                  <>
                                    ...
                                    <div className="flex gap-2">
                                      <button
                                        onClick={() => setCloseForcedMode(false)}
                          ...
                          <div className="bg-black/40 rounded-xl border border-white/5 p-3">
                            ...
                            <button
                              onClick={async () => {
                                ...
                                try {
                                  await eventMutations.diagnoseEvent(selectedEventId, {
                                    user: CURRENT_USER,
                                  });
                                } catch (e) {
                                  console.error(e);
                            ...
                            <p className="text-[10px] text-neutral-600 mt-2 leading-relaxed">
                              El resultado se registrará automáticamente en el timeline.
                            </p>
                          </div>
                        </div>
                      )}
  ...
  const queries = useQueries({
    queries: roots.map((root) => ({
      ...
      queryFn: ({ signal }: { signal?: AbortSignal }) =>
        fetchAffectedCIs(root.id, { signal }),
                  ...
                  <header className="flex items-center justify-between mb-2">
                    <div>
                      <p className="text-sm font-bold text-white">
                        {root.ci_name ?? root.ci_id}
                      </p>
                      <p className="text-[10px] text-neutral-500 uppercase tracking-widest">
                        {root.metric_name ?? root.metric_id} · {root.severity} ·
                        A{sublabelForAffect(root.affected_count)}
                      </p>
                    </div>
