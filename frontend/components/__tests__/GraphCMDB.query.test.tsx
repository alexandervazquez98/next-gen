...
describe("GraphCMDB query ownership", () => {
  beforeEach(() => {
    ...
    Object.defineProperty(SVGElement.prototype, "height", {
      ...
      configurable: true,
      value: {
        baseVal: { ...mockBaseVal, value: 600, valueAsString: "600" },
        animVal: { ...mockAnimVal, value: 600, valueAsString: "600" },
      },
    });
  ...
  it("renders graph node labels from the shared topology query snapshot", async () => {
    mockUseGraphTopologyQuery.mockReturnValue({
      data: {
        nodes: [
          { id: "node-1", label: "Router-01", status: "OK", type: "INFRASTRUCTURE", metadata: {} },
        ],
        links: [],
      },
      isLoading: false,
  ...
  it("keeps owner selections stable across search and collapse controls", async () => {
    ...
    await waitFor(() => {
      expect(getLastFilteredTopologyQuery()).toEqual(
        expect.objectContaining({ owner: ["Platform"] }),
      );
    });
    ...
    await user.click(
      within(screen.getByRole("region", { name: "Owner filters" })).getByRole("button", {
        name: "Clear",
      }),
    );
