...
function buildEventDetail(event: any, node: any, overrides: any = {}) {
  ...
  return {
    ...
    business_context: {
      source:
        metadata.business_service || metadata.impacted_users || metadata.sla_minutes
          ? "snapshot"
          : "unavailable",
...
vi.mock("../../services/api", () => ({
  api: {
    get: vi.fn(async (url: string) => {
      ...
      if (url === `/events/${MOCK_EVENT_WITHIN_SLA.id}`)
        return buildEventDetail(MOCK_EVENT_WITHIN_SLA, MOCK_NODE);
      if (url === `/events/${MOCK_EVENT_WITH_COMMENTS.id}`)
        return buildEventDetail(MOCK_EVENT_WITH_COMMENTS, MOCK_NODE);
...
vi.mock("../../context/AuthContext", () => ({
  useAuth: vi.fn(() => ({
    user: {
      username: "testop",
      role: "OPERATOR",
      permissions: ["EVENT_FORCED_CLOSE"],
      tier: "T2",
      allowed_locations: [],
    },
    hasPermission: (perm: string) =>
      perm === "EVENT_FORCED_CLOSE" || ["EVENT_VIEW", "EVENT_ACK", "EVENT_CLOSE"].includes(perm),
...
describe("M3 — Enriched Timeline", () => {
  ...
  it("GIVEN event with forced close comment WHEN modal opens THEN it renders as forced-close entry", async () => {
    await renderAndOpenModal({
      ...
      id: "evt-force",
      comments: [
        "[AUDIT][FORCED_CLOSE] Cierre forzado por testop\nMotivo: Ventana de mantenimiento (2024-01-01T10:06:00)",
      ],
    });
    expect(
      screen.getAllByText((_, node) => node?.textContent?.includes("Cierre forzado") ?? false)
        .length,
    ).toBeGreaterThan(0);
  });
  ...
  it("GIVEN structured audit entry from another actor WHEN modal opens THEN timeline shows the real actor instead of the viewer", async () => {
    await renderAndOpenModal({
      ...
      id: "evt-audit-actor",
      comments: [
        "[AUDIT][CLOSE] Evento cerrado por alice\nCausa raíz: Error de configuración\nNota: Se corrigió la política de enrutamiento principal (2024-01-01T10:06:00)",
      ],
    });

    expect(
      screen.getAllByText(
        (_, node) => node?.textContent?.includes("alice · Evento cerrado") ?? false,
      ).length,
    ).toBeGreaterThan(0);
    expect(
      screen.queryByText(
        (_, node) => node?.textContent?.includes("testop · Evento cerrado") ?? false,
      ),
    ).toBeNull();
  });
  ...
  it("GIVEN user without EVENT_VIEW but with close and diagnostics permissions WHEN modal opens THEN no detail fetch occurs and authorized actions stay available", async () => {
    ...
    (useAuth as any).mockImplementation(() => ({
      user: {
        username: "closer",
        role: "OPERATOR",
        permissions: ["EVENT_CLOSE", "RUN_DIAGNOSTICS"],
        tier: "T2",
        allowed_locations: [],
      },
    ...
    expect(
      (api.get as any).mock.calls.some(
        (c: any[]) => c[0] === `/events/${MOCK_EVENT_WITHIN_SLA.id}`,
      ),
    ).toBe(false);
    expect(
      (api.get as any).mock.calls.some(
        (c: any[]) => c[0] === `/events/related/${MOCK_EVENT_WITHIN_SLA.ci_id}`,
      ),
    ).toBe(false);
...
describe("M5 — Close with mandatory root cause", () => {
  ...
  it("GIVEN close form WHEN cause + note (≥20 chars) filled THEN Confirmar Cierre is enabled", async () => {
    ...
    fireEvent.change(noteArea, {
      target: { value: "Se reemplazó el módulo de interfaz defectuoso" },
    });
  ...
  it("GIVEN close form filled WHEN Confirmar Cierre clicked THEN close payload contains root cause audit data", async () => {
    ...
    fireEvent.change(screen.getAllByRole("combobox")[1], {
      target: { value: "Error de configuración" },
    });
    fireEvent.change(screen.getByPlaceholderText("Describe la resolución del incidente..."), {
  ...
  it("GIVEN close succeeds WHEN structured close is submitted THEN only the atomic close request is sent", async () => {
    ...
    await waitFor(() => {
      const calls = (api.post as any).mock.calls.filter((c: any[]) =>
        c[0].includes(`/events/${MOCK_EVENT_WITHIN_SLA.id}/`),
      );
  ...
  it("GIVEN close request fails WHEN structured close is submitted THEN no closure comment is written", async () => {
    ...
    fireEvent.change(screen.getAllByRole("combobox")[1], {
      target: { value: "Falla de hardware" },
    });
    fireEvent.change(screen.getByPlaceholderText("Describe la resolución del incidente..."), {
    ...
    await waitFor(() => {
      expect(
        (api.post as any).mock.calls.some(
          (c: any[]) => c[0] === `/events/${MOCK_EVENT_WITHIN_SLA.id}/close`,
        ),
      ).toBe(true);
    });
    ...
    expect(
      (api.post as any).mock.calls.some(
        (c: any[]) => c[0] === `/events/${MOCK_EVENT_WITHIN_SLA.id}/comment`,
      ),
    ).toBe(false);
  ...
  it("GIVEN T2 user WHEN forced close used THEN close payload sends only the operator reason", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Motivo del cierre forzado..."), {
    ...
    await waitFor(() => {
      ...
      const closeCall = calls.find((c: any[]) => c[0].includes("/close"));
      expect(closeCall).toBeDefined();
      expect(closeCall[1].comment_message).toBe(
        "Motivo: Requiere cierre inmediato por ventana de mantenimiento",
      );
    });
  });
});
