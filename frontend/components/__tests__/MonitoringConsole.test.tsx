...
describe('rankCIs', () => {
    ...
    // Helpers
    // -------------------------------------------------------------------------
    const makeNode = (overrides: Partial<GraphNode> & { events?: Event[] }): GraphNode & { events: Event[] } => ({
        id: 'ci-1',
        label: 'Test Node',
        type: 'INFRASTRUCTURE',
        status: 'OK',
    ...
    describe('ranking order', () => {
        it('GIVEN nodes with CRITICAL and WARNING events WHEN ranked THEN CRITICAL nodes rank higher', () => {
            const nodes = [
                makeNode({ id: 'ci-warning', label: 'Warning Node', events: [makeEvent({ severity: 'WARNING', id: 'evt-warn' })] }),
                makeNode({ id: 'ci-critical', label: 'Critical Node', events: [makeEvent({ severity: 'CRITICAL', id: 'evt-crit' })] }),
        ...
        it('GIVEN nodes with multiple events of same severity WHEN ranked THEN total event count determines rank', () => {
            const nodes = [
                makeNode({ id: 'ci-single', label: 'Single', events: [makeEvent({ severity: 'CRITICAL', id: 'evt-1' })] }),
                makeNode({ id: 'ci-triple', label: 'Triple', events: [
        ...
        it('GIVEN mixed severity events per node WHEN ranked THEN weighted sum determines rank', () => {
            const nodes = [
                makeNode({ id: 'ci-2crit', label: '2 Critical', events: [
                    makeEvent({ severity: 'CRITICAL', id: 'evt-a' }),
                    makeEvent({ severity: 'CRITICAL', id: 'evt-b' }),
                ] }),
                makeNode({ id: 'ci-3warn', label: '3 Warning', events: [
    ...
    describe('top-n cap', () => {
        it('GIVEN 10 nodes WHEN top-n=3 THEN returns exactly 3 nodes', () => {
            const nodes = Array.from({ length: 10 }, (_, i) =>
                makeNode({ id: `ci-${i}`, label: `Node ${i}`, events: [makeEvent({ severity: 'CRITICAL', id: `evt-${i}` })] })
            );
        ...
        it('GIVEN n=0 WHEN ranked THEN returns empty array', () => {
            const nodes = [
                makeNode({ id: 'ci-1', events: [makeEvent({ severity: 'CRITICAL' })] }),
            ];
    ...
    describe('stable sort for equal scores', () => {
        it('GIVEN nodes with identical scores WHEN ranked THEN original order is preserved', () => {
            const nodes = [
                makeNode({ id: 'ci-first', label: 'First', events: [makeEvent({ severity: 'INFO', id: 'evt-1' })] }),
                makeNode({ id: 'ci-second', label: 'Second', events: [makeEvent({ severity: 'INFO', id: 'evt-2' })] }),
                makeNode({ id: 'ci-third', label: 'Third', events: [makeEvent({ severity: 'INFO', id: 'evt-3' })] }),
            ];
        ...
        it('GIVEN nodes with equal weighted scores WHEN ranked THEN earlier node wins', () => {
            ...
            const nodes = [
                makeNode({ id: 'ci-2warn', label: '2W', events: [
                    makeEvent({ severity: 'WARNING', id: 'evt-a' }),
                    makeEvent({ severity: 'WARNING', id: 'evt-b' }),
                ] }),
                makeNode({ id: 'ci-4info', label: '4I', events: [
