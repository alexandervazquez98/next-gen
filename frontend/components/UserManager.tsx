            ...
            <div className="flex justify-between items-center mb-6">
              ...
              {editingUser && (
                <button
                  ...
                  className="text-xs text-neutral-400 hover:text-white border border-white/10 px-2 py-1 rounded"
                >
                  CANCEL
                </button>
              )}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    !editingUser && setNewUser({ ...newUser, username: e.target.value })
                  }
              ...
              <div className="border border-white/10 rounded-xl overflow-hidden">
                ...
                >
                  ...
                  <span className="material-symbols-outlined text-sm">
                    {showPerms ? "expand_less" : "expand_more"}
                  </span>
                </button>
                ...
                {showPerms && (
                  <div className="p-3 bg-black/20 max-h-64 overflow-y-auto space-y-4">
                    {[
                      {
                        ...
                        perms: human.filter((p) =>
                          ["EVENT_VIEW", "EVENT_ACK", "EVENT_CLOSE", "EVENT_FORCED_CLOSE"].includes(
                            p,
                          ),
                        ),
                      },
                      {
                        category: "CI Management",
                        perms: human.filter((p) => ["CI_VIEW", "CI_EDIT", "CI_DELETE"].includes(p)),
                      },
                      {
                        category: "Diagnostics",
                        perms: human.filter((p) => p === "RUN_DIAGNOSTICS"),
                      },
                      {
                        category: "System",
                        perms: human.filter((p) =>
                          ["USER_MANAGE", "ROLE_MANAGE", "AUDIT_VIEW"].includes(p),
                        ),
                      },
                      {
                        category: "Visualization",
                        perms: human.filter((p) => p === "METRICS_VIEW"),
                      },
                    ].map((cat) => (
                      <div key={cat.category}>
                        <h5 className="text-[10px] text-neutral-500 font-bold uppercase mb-2">
                          {cat.category}
                        </h5>
                    ...
                    {isAiRole && (
                      <div>
                        <h5 className="text-[10px] text-brand-400 font-bold uppercase mb-2">
                          AI Permissions
                        </h5>
                        <div className="space-y-1">
                          {ai.map((p) => (
