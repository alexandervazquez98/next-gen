...
const GraphCMDB = ({ onNodeClick }: GraphCMDBProps) => {
  ...
  const nodeStateRef = useRef<Map<string, { x: number; y: number; vx: number; vy: number }>>(
    new Map(),
  );
  const clusterOffsetRef = useRef<Map<string, { dx: number; dy: number }>>(new Map());
  ...
  const filteredLocations = useMemo(
    () => allLocations.filter((loc) => loc.toLowerCase().includes(searchLocation.toLowerCase())),
  ...
  const selectedFilterCount = filterLayers.length + filterLocations.length + filterOwners.length;
  const setSelection = (values: string[], setter: Dispatch<SetStateAction<string[]>>) => {
    setter(values);
  };
  ...
  useEffect(() => {
    ...
    const clusterCount = Math.max(
      1,
      new Set(nodes.map((node) => node.location_name || node.owner || node.id)).size,
    );
    const virtualScale = Math.sqrt(clusterCount);
    const width = Math.min(12000, Math.max(viewportWidth, 3200, virtualScale * 760));
    const height = Math.min(8000, Math.max(viewportHeight, 2200, virtualScale * 520));
    ...
    const currentTransform =
      ...
      (width > viewportWidth || height > viewportHeight)
        ? d3.zoomIdentity.translate((viewportWidth - width) / 2, (viewportHeight - height) / 2)
    ...
    const getClusterName = (node: GraphNode) => {
      const raw = (node as any).cluster_name || node.location_name || node.owner || "Unassigned";
      return String(raw);
    };

    const clusterEntries = Array.from(d3.group(nodes, getClusterName).entries()).sort(([a], [b]) =>
      a.localeCompare(b),
    );
    ...
    > = {};
    const clusterGeoQuality = new Map<string, ReturnType<typeof summarizeClusterGeoQuality>>();
    ...
    if (groupByLocation && clusterEntries.length > 0) {
      ...
      const sameCoordinateGroups = d3.group(
        geoLayouts,
        (layout) => `${layout.geo?.lat.toFixed(4)},${layout.geo?.lon.toFixed(4)}`,
      );
      sameCoordinateGroups.forEach((sameGeoLayouts) => {
        sameGeoLayouts.forEach((layout, index) => {
          ...
          const spreadRadius = sameGeoLayouts.length > 1 ? Math.max(110, layout.radius + 44) : 0;
      ...
      if (fallbackLayouts.length > 0) {
        const cols = Math.max(
          1,
          Math.ceil(Math.sqrt(fallbackLayouts.length * (geoLayouts.length > 0 ? 0.65 : 1))),
        );
    ...
    if (groupByLocation) {
      const resolvedClusterCenters = resolveClusterOverlaps(clusterCenters, clusterBounds, {
        padding: 20,
        iterations: 10,
      });
    ...
    if (selectedCluster && groupByLocation && !clusterCenters[selectedCluster]) {
    ...
    const nodeGeoTargets = projectGeoPointsToCanvas(
      nodes
        .filter((node) => isValidGeoCoordinate(node.location?.lat, node.location?.long))
        .map((node) => ({
          id: node.id,
    ...
    const isHighConnectivityNode = (nodeId: string) => (linkCountByNodeId.get(nodeId) || 0) > 4;
    ...
    const isNoisyDetailedLink = (link: any) => {
      ...
      const sourceId = getLinkEndpointId(link.source);
      const targetId = getLinkEndpointId(link.target);
      return isHighConnectivityNode(sourceId) || isHighConnectivityNode(targetId);
    };
    ...
    const visibleClusterLinks = selectedCluster
      ? clusterLinks.filter(
          (link) => link.source === selectedCluster || link.target === selectedCluster,
        )
      : clusterLinks;
    ...
    const nodeMatchesSearch = (node: any) => {
      ...
      return searchableText(searchPayload).toLowerCase().includes(normalizedGraphSearch);
    };
    const matchingNodeIds = new Set(
      validNodes.filter((node: any) => nodeMatchesSearch(node)).map((node: any) => node.id),
    );
    ...
    const hasSearchMatches = normalizedGraphSearch.length > 0 && matchingNodeIds.size > 0;
    ...
    clusterSelection?.append("title").text(([name]) => clusterQualityTooltip(name));
    ...
    clusterSelection
      ...
      .attr("fill-opacity", ([name]) =>
        matchingClusterNames.has(name) ? 0.46 : selectedCluster === name ? 0.38 : 0.24,
      )
      .attr("stroke", ([name]) =>
        matchingClusterNames.has(name)
          ? "#facc15"
      ...
      .attr("stroke-width", ([name]) =>
        matchingClusterNames.has(name) ? 4 : selectedCluster === name ? 2.5 : 1.5,
      )
      .attr("filter", ([name]) =>
    ...
    clusterBadgeSelection
      ?.append("text")
      .attr("text-anchor", "middle")
      .attr("fill", ([name]) => (clusterCenters[name]?.hasGeo ? "#fbbf24" : "#fb7185"))
      .attr("font-size", "10px")
      .attr("font-weight", 800)
    ...
    const clusterLinkSelection = container
      ...
      .attr("opacity", (link) => {
        if (hasSearchMatches) {
          return matchingClusterNames.has(link.source) || matchingClusterNames.has(link.target)
            ? 0.78
            : 0.08;
        }
    ...
    const simulation = d3
      ...
      .force(
        "charge",
        d3.forceManyBody().strength(clusterMode ? -8 : groupByLocation ? -140 : -420),
      )
      .force("center", d3.forceCenter(width / 2, height / 2).strength(clusterMode ? 0 : 0.02))
      .force("collision", d3.forceCollide().radius(GRAPH_NODE_COLLISION_RADIUS).iterations(2))
      .force(
        "x",
        d3
          .forceX()
    ...
    const linkSelection = container
      ...
      .attr("stroke", (link: any) => {
        const targetNode = getRenderedEndpoint(link.target) as GraphNode | undefined;
    ...
    const trafficLink = container
      .append("g")
      .selectAll("path")
      .data(detailedLinks.filter((link: any) => link.relationship === "CONNECTS_TO"))
      .join("path")
      .attr("fill", "none")
      .attr("stroke-width", 3)
      .attr("stroke", "#10b981")
    ...
    const nodeSelection = container
      ...
      .attr("opacity", (node) => {
        ...
        return matchingNodeIds.has(node.id) || matchingClusterNames.has(node.clusterName)
          ? 1
          : 0.18;
      })
    ...
    nodeSelection
      ...
      .attr("fill", (node) => (matchingNodeIds.has(node.id) ? "#3a2f08" : "#1a1a1a"))
      .attr("stroke", (node) => {
    ...
    nodeSelection
      ...
      .text((node: any) =>
        selectedCluster === node.clusterName && node.isGeoOutlier ? "geo outlier" : "",
      );
    ...
    const renderPositions = () => {
      enforceContainment();
      clusterSelection?.attr("transform", ([, center]) => `translate(${center.x},${center.y})`);
      clusterLinkSelection
        .selectAll(
    ...
    const moveClusterWithMembers = (name: string, dx: number, dy: number) => {
      ...
      const center = clusterCenters[name];
      if (!center) return;
      const boundedDelta = getBoundedClusterDelta(center, dx, dy, clusterBounds);
        ...
        for (let i = 0; i < entries.length; i++) {
          for (let j = i + 1; j < entries.length; j++) {
            ...
            if (nameA === activeName) {
              ...
              moveClusterWithMembers(nameA, (-ux * overlap) / 2, (-uy * overlap) / 2);
              moveClusterWithMembers(nameB, (ux * overlap) / 2, (uy * overlap) / 2);
            }
            moved = true;
          }
        }
    ...
    nodeSelection
      .on("mouseover", (_event, d) => {
        ...
        nodeSelection.attr("opacity", (node) => (connectedNodeIds.has(node.id) ? 1 : 0.18));
        nodeSelection
          .select("circle")
          .attr("stroke-width", (node) => {
          ...
          .attr("filter", (node) =>
            connectedNodeIds.has(node.id) ? "drop-shadow(0 0 8px currentColor)" : null,
          );
        nodeSelection
          .select(".node-label")
          .attr("fill", (node) => (connectedNodeIds.has(node.id) ? "white" : "#a3a3a3"))
          .attr("font-weight", (node) => (connectedNodeIds.has(node.id) ? "bold" : "normal"))
          .text((node) =>
        ...
        linkSelection.attr("opacity", (link: any) => (isConnectedLink(link) ? 0.9 : 0.08));
        trafficLink.attr("opacity", (link: any) => (isConnectedLink(link) ? 0.95 : 0.08));
      })
      .on("mouseout", () => {
        ...
        linkSelection.attr("opacity", (link: any) => getDefaultDetailedLinkOpacity(link, 0.6));
        trafficLink.attr("opacity", (link: any) => getDefaultDetailedLinkOpacity(link, 0.7));
      });
    ...
    function dragended(event: any) {
      ...
      const next = clampToCluster(event.subject, event.subject.x, event.subject.y);
      event.subject.x = next.x;
      event.subject.y = next.y;
      event.subject.vx = 0;
      event.subject.vy = 0;
  ...
  }, [links, nodes, onNodeClick, groupByLocation, selectedCluster, graphSearch]);
    ...
    <div className="w-full h-full relative overflow-hidden bg-surface-950 grid-bg flex">
        ...
        <div>
          ...
          <div className="space-y-4">
            ...
            {selectedFilterCount > 0 && (
              <div className="flex flex-wrap gap-1" aria-label="Selected graph filters">
            ...
            <section
              ...
              className="rounded-xl border border-white/5 bg-neutral-950/60"
            >
              <button
                type="button"
              ...
              >
                <span className="text-[10px] font-bold text-neutral-400 uppercase">Technology</span>
            ...
            >
              ...
              >
                <span className="text-[10px] font-bold text-neutral-400 uppercase">Owner</span>
                <span className="text-[10px] text-neutral-500">
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute bottom-4 left-4 flex flex-col gap-2 p-3 glass rounded-lg text-xs pointer-events-none select-none">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-brand-500 rounded-full"></div> Operational
          </div>
          <div className="flex items-center gap-2">
