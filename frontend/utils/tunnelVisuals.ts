	...
	for (const character of value) {
		...
		if (codePoint <= UTF8_SINGLE_BYTE_LIMIT) {
			...
			bytes.push(
				...
				UTF8_CONTINUATION_PREFIX | ((codePoint >> BITS_PER_UTF8_CONTINUATION_BYTE) & UTF8_PAYLOAD_MASK),
				UTF8_CONTINUATION_PREFIX | (codePoint & UTF8_PAYLOAD_MASK),
			);
		} else {
			bytes.push(
				UTF8_FOUR_BYTE_PREFIX | (codePoint >> (BITS_PER_UTF8_CONTINUATION_BYTE * 3)),
				UTF8_CONTINUATION_PREFIX | ((codePoint >> (BITS_PER_UTF8_CONTINUATION_BYTE * 2)) & UTF8_PAYLOAD_MASK),
				UTF8_CONTINUATION_PREFIX | ((codePoint >> BITS_PER_UTF8_CONTINUATION_BYTE) & UTF8_PAYLOAD_MASK),
				UTF8_CONTINUATION_PREFIX | (codePoint & UTF8_PAYLOAD_MASK),
			);
		}
...
function base64UrlEncodeUtf8(value: string): string {
	...
	for (let index = 0; index < bytes.length; index += BASE64_CHUNK_BYTE_COUNT) {
		...
		const chunk = (first << BASE64_CHUNK_FIRST_BYTE_SHIFT) | ((second ?? 0) << BASE64_CHUNK_SECOND_BYTE_SHIFT) | (third ?? 0);
		...
		if (second !== undefined) encoded += BASE64_URL_ALPHABET[(chunk >> BASE64_SEXTET_SHIFTS[2]) & BASE64_SEXTET_MASK];
