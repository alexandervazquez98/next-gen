...
export const TUNNEL_HEALTH_TELEMETRY_FLUSH_INTERVAL_MS =
  TUNNEL_HEALTH_TELEMETRY_WINDOW_SECONDS * 1_000;
...
const emptyLatencyBucket = (): LatencyBucket => ({
  lt_250: 0,
  lt_1000: 0,
  lt_5000: 0,
  gte_5000: 0,
...
export function buildTunnelHealthTelemetryPayload(
  input: TunnelHealthTelemetryInput,
): TunnelHealthTelemetryPayload {
  return {
...
function hasDiagnosticTelemetry(input: TunnelHealthTelemetryInput): boolean {
  return (
    hasFailureTelemetry(input) ||
    (input.suppressedCooldown ?? 0) > 0 ||
    Boolean(input.killSwitchEnabled)
  );
}
...
export function createTunnelHealthTelemetryBatcher({ now = () => Date.now() } = {}) {
  ...
  return {
    async flush(input: TunnelHealthTelemetryInput): Promise<boolean> {
      ...
      const diagnosticEscalation =
        hasDiagnostics && diagnosticKinds.some((kind) => !lastFlushDiagnosticKinds.has(kind));
      if (
        current - lastFlushMs < TUNNEL_HEALTH_TELEMETRY_FLUSH_INTERVAL_MS &&
