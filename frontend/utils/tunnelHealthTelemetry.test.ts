...
describe("tunnel health telemetry", () => {
  ...
  it(
    "builds aggregate-only payloads without link ids, endpoints, URLs, IPs, or per-link arrays",
    () => {
      ...
      expect(JSON.stringify(payload)).not.toMatch(
        /link_id|hub-a|edge-b|\/tunnels\/|203\.0\.113\.10|192\.168\.1\.1|\[/,
      );
    },
  );
  ...
  it("flushes at most once per minute and only when active or failing", async () => {
    ...
    expect(
      await batcher.flush({ scheduled: 0, success: 0, failureByKind: { network: 1 } }),
    ).toBe(true);
  ...
  it(
    "does not let an early scheduled-only flush hide later failure or cooldown telemetry",
    async () => {
      let now = 0;
      mockApiPost.mockResolvedValue({ accepted: true });
  ...
  it("does not let earlier failure diagnostics hide later cooldown-only telemetry", async () => {
    ...
    expect(
      await batcher.flush({ scheduled: 1, success: 0, failureByKind: { server: 1 } }),
    ).toBe(true);
    now = 1_000;
